...
type PeriodKey = 'today' | 'this_week' | 'this_month' | 'this_quarter' | 'this_year'
  ...
  return useAsyncData<StrategyWithAccounts[]>(
    ...
    async () => {
      ...
      const [
        ...
        { data: yearSnapshotData, error: yearSnapshotError }
      ] = await Promise.all([
        supabase
          .from('strategy_snapshots')
        ...
        supabase
          .from('strategy_snapshots')
          .select(snapshotColumns)
          .gte('snapshot_ts', yearStart.toISOString())
          .lte('snapshot_ts', latestSnapshot.snapshot_ts)
          .order('snapshot_ts', { ascending: true })
          .order('strategy_id', { ascending: true })
          .in('strategy_id', strategyIds)
      ...
      if (yearSnapshotError) throw yearSnapshotError
      ...
      const baselinesByPeriod = {
        ...
        this_year: buildBaselineRows(StrategySnapshotRowsSchema.parse(yearSnapshotData ?? []))
      }
